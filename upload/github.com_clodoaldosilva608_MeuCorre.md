# GitHub - clodoaldosilva608/MeuCorre: ⚡ MeuCorre — PWA para entregadores de aplicativo (iFood, 99Food, Lalamove) controlarem corridas, despesas e lucro líquido. 100% offline, Local-First, com painel admin e integração Kiwify. · GitHub

**URL:** https://github.com/clodoaldosilva608/MeuCorre
...
Latest commit
clodoaldosilva608
fix: capacetes alinhados horizontalmente no mobile
5981e22
 · 
History
163 Commits
...
810e39da-f2c1-48d7-8551-f41544dc5581
	


agent-ctx
	
52261aa6-e9ac-4715-90e6-4dd62c9f85e1
...
feat(security): correções P0 + P1 — aplicação 100% segura para lançam…
	


prisma
	
fix: liberar geolocation + implementar quiz de captação de leads
	


public
	
fix: capacetes parados + alinhados horizontalmente + novas imagens
...
fix: remover fundo branco dos capacetes (canal alpha)
...
fix: cor do TikTok ajustada + espaçamento maior
	


.env.example
	
feat: preparação para 100k usuários (P0 + P1 + P2 + P3 + P4)
	


.gitignore
	
feat: admin blog + pop-up aprovação Blogger + API blog
...
docs(visual): 12 screenshots + README atualizado + smoke test fix
	


bun.lock
	
feat: redesign premium enterprise (Dark Glassmorphism Tech)
...
fix: corrigir erros de ESLint no use-work-sessions.ts
	


package-lock.json
	
feat: mapa de calor + análise lead generation + testes aprovados
...
fix: 6 correções da auditoria de engenharia (P0 + P1)
...
fix: atualizar redes sociais + remover prisma db push do build
...
Princípios
Local-First: dados do entregador (corridas, despesas) ficam no celular — zero servidor, máxima privacidade
Server-Side Admin: anúncios, assinaturas e feedbacks ficam no Postgres (Supabase) pra persistência e gestão centralizada
PWA Instalável: funciona como app nativo, sem burocracia de lojas
Auto-ativação de licença: webhook Kiwify → licença gerada → cliente redirecionado → app ativa automaticamente
🛠️ Stack Tecnológica
Camada	Tecnologia
...
Configure as env vars (lista acima)
Deploy — Vercel instala deps, roda prisma generate && next build, faz deploy
Opção 2: Vercel CLI
...
📖 Manual de Uso — App do Entregador
1. Acessando o app
Abra https://meucorre.vercel.app/app no navegador do celular
Instale como app: Chrome (Android) → menu ⋮ → "Adicionar à tela inicial" | Safari (iOS) → Compartilhar → "Adicionar à Tela de Início"
O ícone ⚡ MeuCorre aparece junto dos outros apps
...
Toque no botão + (verde, canto inferior direito)
Selecione o app (iFood, 99Food, Lalamove, etc.) — toque no card
Digite o valor (ou use os botões rápidos R$ 5/10/15/20/25/30)
Digite os km rodados (opcional)
Adicione uma observação se quiser (ex: "bairro Centro")
Toque em Lançar Corrida
5. Captura por notificação (atalho)
Toque no sininho 🔔 no header
Cole o texto da notificação do app (ex: "iFood: Pedido entregue! Você recebeu R$ 15,50")
O MeuCorre extrai automaticamente o app e o valor
Confirme e lance
6. Lançando uma despesa
Toque na tab Despesas (inferior)
Toque no botão + (vermelho)
Selecione a categoria (Combustível ⛽, Alimentação 🍔, Manutenção 🔧, Bateria 🔋, Pedágio 🚧, Outros 💸)
Digite o valor
Adicione descrição (opcional)
Toque em Lançar Despesa
7. Vendo gráficos
...
Compre na landing page (/)
Após pagamento, você é redirecionado pra /obrigado e depois pro /app com a licença ativada automaticamente
Se não ativou sozinho: toque no 👑 Coroa no header → cole a licença (veio por email) → Ativar PRO
Pronto! Badge "PRO" aparece no header, anúncios somem, features liberadas
...
Anúncios (/admin/ads)
Criar novo anúncio
Toque em Novo anúncio
Selecione o placement:
...
Toque em Revisar num item pendente
Veja o comprovante (se foi upload manual)
Adicione notas (visíveis ao cliente)
Toque em Aprovar e gerar licença
Licença crypto 32-hex é gerada automaticamente
Copie com o botão Copiar
Envie pro cliente (por email ou WhatsApp)
...
Backup do Postgres: Supabase faz backup automático diário (plano grátis: 7 dias)
O NÃO fazer
...
Quem pode indicar: Apenas usuários PRO (assinatura vitalícia ativa) podem gerar links de indicação.
...
Recompensa: R$5,00 por cada amigo que se tornar PRO, creditados via PIX.
...
Fraude: Tentativas de fraude (múltiplas contas, auto-indicação, indicações falsas) resultam em cancelamento da participação no programa e bloqueio da conta.

Ativação/Desativação: O administrador pode ativar ou desativar a campanha a qualquer momento pelo painel admin. Indicações já feitas continuam válidas, mas novas indicações não geram recompensa quando a campanha está inativa.
...
⚡ MeuCorre — PWA para entregadores de aplicativo (iFood, 99Food, Lalamove) controlarem corridas, despesas e lucro líquido. 100% offline, Local-First, com painel admin e integração Kiwify.

meucorre-clodoaldosilva608.vercel.app
Resources
Readme
Activity
Stars
