...
import type { Delivery, PeriodFilter as Period, PeriodStat } from "@/lib/types";
...
export function ReferenceDashboard({ stats, period, activeTab, goal, recentDeliveries, onPeriodChange, onNewDelivery, onStartSession, onOpenHeatmap, onTabChange, onOpenApps, onOpenCapture, onOpenLicense, onOpenShare, onOpenOnboarding, onExportJSON, onExportCSV, onClearAll, onLogout }: ReferenceDashboardProps) {
  ...
  return (
    <section className="reference-dashboard" aria-label="Dashboard MeuCorre">
      ...
      <div className="reference-dashboard-body">
        ...
        <section className="reference-dashboard-card reference-apps-card"><header><h2>Resumo por app</h2><button type="button" onClick={() => onTabChange("corridas")}>Ver todos</button></header><div className="reference-app-grid">{stats.byApp.length === 0 ? <p className="text-xs text-zinc-500">Nenhum app com corridas neste período.</p> : stats.byApp.map((app) => <div key={app.app}><span className="reference-app-icon" style={{ background: app.color }}>{app.label.slice(0, 2)}</span><strong>{app.label}</strong><b>{formatBRL(app.total)}</b><small>{app.count} {app.count === 1 ? "corrida" : "corridas"}</small></div>)}</div></section>
        <section className="reference-dashboard-card reference-recent-card"><header><h2>Últimas corridas</h2><button type="button" onClick={() => onTabChange("corridas")}>Ver todas</button></header>{recentDeliveries.length === 0 ? <p className="text-xs text-zinc-500">Nenhuma corrida registrada neste período.</p> : recentDeliveries.slice(0, 5).map((delivery) => <button type="button" className="reference-recent-row" key={delivery.id} onClick={() => onTabChange("corridas")}><span><b>{new Intl.DateTimeFormat("pt-BR", { hour: "2-digit", minute: "2-digit" }).format(new Date(delivery.timestamp))}</b><small>{delivery.date}<br />{delivery.address || delivery.app}</small></span><strong>{formatBRL(delivery.value)}</strong><ChevronRight /></button>)}</section>
