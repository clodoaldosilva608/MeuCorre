...
import { PLAN_PRICE, MONTHLY_PRICE, ANNUAL_PRICE } from "@/lib/commercial-cta";
  ...
  return (
    <section className="reference-landing" aria-label="Landing page MeuCorre">
      <div className="reference-landing-inner">
        ...
        <section id="planos" className="reference-landing-section"><h2>Escolha o plano ideal para você</h2><div className="reference-card-grid four reference-pricing"><article><h3>Free</h3><p>Tudo que você precisa para começar.</p><strong>{brl(0)}</strong><small>pra sempre</small><ul><li><Check /> Corridas ilimitadas</li><li><Check /> Despesas</li><li><Check /> Metas diárias</li><li><Check /> 100% offline</li></ul><Link href={isAuthenticated ? "/app" : "/quiz"}>Usar grátis</Link></article><article className="featured"><span>MAIS ESCOLHIDO</span><h3>PRO</h3><p>Mais controle, mais análises, mais resultado.</p><strong>{brl(MONTHLY_PRICE)}</strong><small>/mês</small><ul><li><Check /> Tudo do plano Free</li><li><Check /> Gráficos avançados</li><li><Check /> Relatórios completos</li><li><Check /> Suporte prioritário</li></ul><button type="button" onClick={() => onCheckout("monthly")}>Testar PRO</button></article><article><h3>Anual</h3><p>Economize 46% pagando no ano.</p><strong>{brl(ANNUAL_PRICE)}</strong><small>/ano</small><ul><li><Check /> Tudo do plano PRO</li><li><Check /> Atualizações por 1 ano</li><li><Check /> Suporte prioritário</li><li><Check /> Economia de R$ 81/ano</li></ul><button type="button" onClick={() => onCheckout("annual")}>Quero anual</button></article><article><h3>Vitalício</h3><p>Pague uma vez e use para sempre.</p><strong>{brl(PLAN_PRICE)}</strong><small>pagamento único</small><ul><li><Check /> Tudo do plano PRO</li><li><Check /> Atualizações vitalícias</li><li><Check /> Sem mensalidade</li><li><Check /> Acesso eterno</li></ul><button type="button" onClick={() => onCheckout("lifetime")}>Quero vitalício</button></article></div></section>
